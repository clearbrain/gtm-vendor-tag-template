...

{
  ...
  "description": "Record user events, such as a page loads, auth, and custom events.  This tag sends recorded events to the ClearBrain API directly.",
  "containerContexts": [
    "WEB"
  ]
}
...
[
  {
    ...
    "notSetText": "(Enter the write key specified in the Connections detail pane.)",
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      },
    ...
    "help": "The 8-10 character write key which identifies your connection to ClearBrain.  It is issued in the ClearBrain app, and is found in the Connections detail pane for each GTM connection.",
  ...
  },
  {
    ...
    "help": "The type of event to send to ClearBrain.  `page` calls represent a page view in the application and should specify a `name` parameter.  `track` calls specify an arbitrary event of any kind and should specify an `event` parameter. `identify` calls link a `user_id` to an `anonymous_id` and add traits to user."
  },
  {
    "type": "SIMPLE_TABLE",
    "name": "parameters",
    ...
    "help": "Parameters associated with the event.  Each event must at least specify the user associated with it as a `user_id` (logged in) or an `anonymous_id` (visitor) -- both may be specified in an `identify` call."
  }
]
...
const baseUrl = 'https://api.clearbrain.com/v1/pixel/' + eventType + '?write_key=' + writeKey;
...
Created on 8/20/2019, 2:29:54 PM
